...
const styles = {
  wrapper: { minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#f1f5f9", fontFamily: "sans-serif" },
  card: { background: "white", borderRadius: 12, padding: "40px 36px", width: 360, boxShadow: "0 4px 24px rgba(0,0,0,0.08)" },
  ...
  input: { width: "100%", padding: "10px 12px", borderRadius: 7, border: "1.5px solid #d1d5db", fontSize: 15, boxSizing: "border-box", marginBottom: 16, outline: "none" },
  btn: { width: "100%", padding: "11px 0", borderRadius: 7, border: "none", background: "#1e40af", color: "white", fontWeight: 700, fontSize: 15, cursor: "pointer", marginBottom: 12 },
  ...
  return (
    <div style={styles.wrapper}>
      <div style={styles.card}>
        ...
        <input style={styles.input} type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="votre@email.com" onKeyDown={(e) => e.key === "Enter" && (isSignup ? handleSignup() : handleLogin())} />
        <label style={styles.label}>Mot de passe</label>
        <input style={styles.input} type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••••••" onKeyDown={(e) => e.key === "Enter" && (isSignup ? handleSignup() : handleLogin())} />
